...
if __name__ == "__main__":
	...
	documents = []
	totalDocuments = 0
	for (at, loaded_persons) in chunk_query(mysql, person_query, chunksize=1000):
		cards = {}
		persons = {}
		errors = 0
		...
		for person_id in persons:
			...
			try:
				data = {
					...
					'civilstand': "Gift" if person['married'] == 1 or ('spouses' in card and len(card['spouses']) > 0) else "Ugift",
			...
			documents.append({
				...
				'civilstatus': "Gift" if person['married'] == 1 or ('spouses' in person and len(person['spouses']) > 0) else "Ugift",
				'person_id': person_id,
