...
class PagesModelsTest extends \UnitTestCase {
    ...
    {
        $this->getDI()->get('database')->execute('DROP TABLE IF EXISTS apacs_pages');
        $createQuery = 'CREATE TABLE apacs_pages (id INT(11) AUTO_INCREMENT PRIMARY KEY, collection_id INT(11) NOT NULL, concrete_unit_id INT(11) NOT NULL, concrete_page_id INT(11) NOT NULL, tablename CHAR(50) NOT NULL)';
    ...
    {
        $this->getDI()->get('database')->execute('DROP TABLE IF EXISTS test_page');
        $createQuery = 'CREATE TABLE test_page (id INT(11) AUTO_INCREMENT PRIMARY KEY, collection_id INT(11) NOT NULL, volume_id INT(11) NOT NULL)';
        $this->getDI()->get('database')->execute($createQuery);
        
        $insert = 'INSERT INTO test_page (id,collection_id, volume_id) VALUES (1,1,43), (2, 1,44)';
    ...
    public function testCreatePages()
    {
        ...
        $imp = new Pages();
        $imp->Import(Pages::OPERATION_TYPE_CREATE, 1, 'id', 'volume_id', 'test_page');
    ...
    public function testUpdatePages()
    {
        ...
        //Importing data
        $imp = new Pages();
        $this->assertTrue($imp->Import(Pages::OPERATION_TYPE_CREATE, 1, 'id', 'volume_id', 'test_page'), 'should import data');

        //Changing original data
        $this->getDI()->get('database')->execute('update test_page set info = "desc3" WHERE id = 1 LIMIT 1');
      
        //Updating data
        $this->assertTrue($imp->Import(Pages::OPERATION_TYPE_UPDATE, 1, 'id', 'volume_id', 'test_page'), 'should update without errors');
