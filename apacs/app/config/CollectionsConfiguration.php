...
$collectionsSettings = array(
	...
	array(
		...
		'objects_query' => "select files.id, CONCAT('$cdnUrl', 'collections/mandtal',path) as imageURL, year, month, name FROM files LEFT JOIN volumes ON files.volume_id = volumes.id LEFT JOIN streets on volumes.street_id = streets.id WHERE :query ORDER BY year, month, files.id",
	...
	array(
		...
		'primary_table_name' => 'kortteg_files',
		//How to link the data level objects to images
		'objects_query' => "SELECT SkoleKildeOpslagId as id, Navn, Kildenavn, kilde.AarstalTil, kilde.AarstalFra, skole.SkoleId as skoleid, skole.navn as skole, opslag.Kildeid as kildeid, kilde.SkoleKildeID as SkoleKildeId, CONCAT('$cdnUrl', 'collections/skoleprotokoller/',FuldFilNavn) as imageURL
