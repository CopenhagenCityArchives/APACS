...
$di->setShared('ServerEventLogger', function () use ($startTime) {
	return new ServerEventLogger($startTime);
});
...
$eventsManager->attach('micro', function () {});
...
$app->after(function () use ($di) {
	$di->get('ServerEventLogger')->SetEvent('test', null);
	$di->get('ServerEventLogger')->Save();
});
...
try {
	...
	$di->get('response')->setContent("Exception caught in bootstrapper: " . $e->getMessage());
	$di->get('response')->send();
}