...
class DBConfigurationLoader {
	...
	public function GetTaskFieldsSchema($taskId) {
		/*$data = [];
			...
			$entities = Entities::find(['conditions' => 'task_id = ' . $taskId, 'hydration' => Resultset::HYDRATE_ARRAYS]);
			$fields = new Tasks();
			$data['schema'] = [];
		...
		*/
		...
		$data = $entity->GetEntityAndFields(1);

		return ($entity->getEntityAsJSONSchema($data));
	}
 ...
 */
	...
	public function GetSteps($taskId) {
		...
		$steps = Steps::find(['condition' => 'tasks_id = ' . $taskId, 'columns' => ['id', 'name', 'description']]);
		...
		foreach ($steps as $step) {
			...
			$data[$i]['fields'] = [];
			$result = new Query('SELECT e.guiName, ef.id, e.countPerEntry FROM EntitiesFields ef LEFT JOIN Entities e ON ef.entity_id = e.id WHERE ef.step_id = ' . $step->id, \Phalcon\DI\FactoryDefault::getDefault());
